...
class _HomeState extends State<Home> {
  ...
  CollectionReference recipeStream = FirebaseFirestore.instance.collection("recipes");
  CollectionReference preferenceCollection = FirebaseFirestore.instance.collection("preferences");
  ...
  num cookTimeMin = 0.0;
  num cookTimeMax = 500.0;
  num caloriesMin = 0.0;
  num caloriesMax = 10000.0;
  num sugarMin = 0.0;
  num sugarMax = 1000.0;
  num proteinMin = 0.0;
  num proteinMax = 1000.0;
  num sodiumMin = 0.0;
  num sodiumMax = 5000.0;
  num fatMin = 0.0;
  num fatMax = 1000.0;
  ...
  StreamController<List<DocumentSnapshot>> controller = StreamController<List<DocumentSnapshot>>();
  ...
  Future setDefaultPreferenceValues() async{
    await preferenceCollection.doc(widget.uid).set({
      "isVeganOnly": false,
      "cookTimeMin": 0.0,
      "cookTimeMax": 500.0,
      "caloriesMin": 0.0,
  ...
  Future<void> fetchPreferences() async {
    try {
      await preferenceCollection.doc(widget.uid).get().then((value) async {
        if (value.data().toString() == '{}' || value.data().toString() == 'null') {
          await setDefaultPreferenceValues();
          fetchPreferences();
        }else{
  ...
  getRecipes() async {
    ...
    if (firstCall == true) {
      for (var id in preferredRecipesIds) {
        querySnapshot = await db.collection("recipes").where("RecipeId", isEqualTo: id).get();
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Scaffold(
                                    ...
                                    decoration: BoxDecoration(
                                      ...
                                      boxShadow: const [
                                        BoxShadow(
                                          ...
                                          spreadRadius: 3,
                                          blurRadius: 10,
                                          offset: Offset(3.5, 7), // changes position of card shadow
                                        ),
                                      ],
                                    ),
                                  ),
        ...
        extendBody: true,
        //to make navbar notch transparent
